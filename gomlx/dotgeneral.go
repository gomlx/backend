...
func dgSelectExecPath(backend *Backend, lhsShape, rhsShape shapes.Shape, params *dotGeneralNodeData) dotGeneralExecutionPath {
	...
	if backend.dotGeneralForceExecutionPath != autoSelectPath {
		...
		case highwayPath:
			...
			valid = highwayHasDTypeSupport(dtype, dtype) &&
				isMatMulOrder(lhsShape, params.lhsContractingAxes, params.lhsBatchAxes,
					rhsShape, params.rhsContractingAxes, params.rhsBatchAxes)
		default:
			valid = true
	...
	canHighway := canUseHighwayPath(lhsShape, params.lhsContractingAxes, params.lhsBatchAxes,
		rhsShape, params.rhsContractingAxes, params.rhsBatchAxes)
	if highwayHasDTypeSupport(dtype, dtype) && canHighway {
		return highwayPath
	}
...
func execDotGeneral(backend *Backend, node *Node, inputs []*Buffer, _ []bool) (*Buffer, error) {
	...
	case blockedPath, checkPath:
		...
		if err == nil && params.execPath == checkPath {
			...
			if isMatMulOrder(lhsRaw.shape, params.lhsContractingAxes, params.lhsBatchAxes,
				rhsRaw.shape, params.rhsContractingAxes, params.rhsBatchAxes) &&
				highwayHasDTypeSupport(inputDType, inputDType) {
				err = highwayMatMulDynamic(inputDType, outputShape.DType, lhsRaw.flat, rhsRaw.flat,
					params.batchSize, params.lhsCrossSize, params.rhsCrossSize, params.contractingSize,
					output2.flat,
	...
	case highwayPath:
		...
		if lhsNeedsTranspose == noTranspose && rhsNeedsTranspose == needs2DTranspose {
			err = highwayMatMulKLast(inputDType, outputDType, lhs.flat, rhs.flat,
				params.batchSize, params.lhsCrossSize, params.rhsCrossSize, params.contractingSize,
		...
		err = highwayMatMulDynamic(inputDType, outputDType, lhsFlat, rhsFlat,
			params.batchSize, params.lhsCrossSize, params.rhsCrossSize, params.contractingSize,
			output.flat,
			getAnyBufAllocator(backend, inputDType), getBufReleaser(backend), backend.workers)
