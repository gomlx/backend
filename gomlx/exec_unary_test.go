...
func TestExecUnary_Exp(t *testing.T) {
	...
	// Tolerance relaxed from 1e-15 to 1e-6: highway SIMD path trades precision for throughput.
	assert.InDelta(t, 2.718281828459045, y1.Value(), 1e-6)
	y2 := exec.MustExec(bfloat16.FromFloat32(1.0))[0]
	want := bfloat16.FromFloat32(float32(math.E)).Float32()
	// Tolerance relaxed from 1e-2 to 0.02: highway SIMD path trades precision for throughput.
	assert.InDelta(t, want, y2.Value().(bfloat16.BFloat16).Float32(), 0.02)
}
...
func TestExecUnary_Log(t *testing.T) {
	...
	// Tolerance relaxed from 1e-15 to 1e-9: highway SIMD path trades precision for throughput.
	assert.InDelta(t, 1.0, y1.Value(), 1e-9)
